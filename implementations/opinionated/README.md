...
```bash
set -a; . ../../.env; set +a            # load OPENAI_API_KEY / ANTHROPIC_API_KEY
...
```bash
docker build -t opinionated-memory .
# default container port 8080; for a parallel run use host port 8091:
HOST_PORT=8091 OPENAI_API_KEY=... ANTHROPIC_API_KEY=... docker compose up --build
until curl -sf http://localhost:8091/health; do sleep 1; done
```
